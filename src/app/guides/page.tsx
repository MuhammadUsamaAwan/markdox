...
export default function GuidesPage() {
  ...
  return (
    <div className='py-6 lg:py-10'>
      <DocsPageHeader heading='Guides' text='This section includes end-to-end guides for developing Next.js 13 apps.' />
      {guides?.length ? (
        <div className='grid gap-4 md:grid-cols-2 md:gap-6'>
          {guides.map(guide => (
            ...
            >
              {guide.featured && (
                <span className='absolute right-4 top-4 rounded-full px-3 py-1 text-xs font-medium'>Featured</span>
              )}
              <div className='flex flex-col justify-between space-y-4'>
                ...
                {guide.date && <p className='text-sm text-muted-foreground'>{formatDate(guide.date)}</p>}
              </div>
